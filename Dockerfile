...
RUN apt-get update && apt-get install -y git python3.7 python3-pip libpython3.7-dev && python3.7 -m pip install --upgrade pip virtualenv && mkdir -p /srv
WORKDIR /srv
...
ENTRYPOINT ["/srv/env/bin/python", "manage.py"]
CMD ["runserver", "0:80"]
EXPOSE 80